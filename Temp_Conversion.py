...
if user_input == 'a' :
    ...
    if is_numeric(Temp_value_in_Fahrenheit):
         converted_temp_c = (float(Temp_value_in_Fahrenheit) - 32) * 5/9
     ...
     if is_numeric(Temp_in_Celsius):
          converted_temp_f = (float(Temp_in_Celsius) * 9/5) + 32
          converted_temp_k = float(Temp_in_Celsius) + 273.15
          print()
    ...
    if is_numeric(Temp_value_in_Kelvin):
        converted_temp_c = float(Temp_value_in_Kelvin) - 273.15
        converted_temp_f = (float(converted_temp_c) * 9/5) + 32
        print()
